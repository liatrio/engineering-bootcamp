...
- Create a backstage template to onboard an 'app teams' application. Your template should do the following.
  ...
  - Create a PR to your deploy repo that adds `prod/applications/<app-name>/kustomization.yaml` and `prod/applications/<app-name>/application.yaml`
    > Your application.yaml should point to `prod/applications/<app-name>` at HEAD in your deploy repo. Your kustomization.yaml should point to
      the app teams repo at the version specified in the template.
    ...
    > If you take this route you will need a way to get the secrets from the secrets manager to your cluster. Research how you can getKKubernetes to fetch secrets from a secrets manager.
...
2) Declaratively configure an ArgoCD Credential Template leveraging new secrets.
3) Add the secrets needed to pull from the private container registry to your secrets manager.
4) Configure Kubernetes to get the ImagePullSecret.
