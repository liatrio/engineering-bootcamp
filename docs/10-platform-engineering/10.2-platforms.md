...
- Create the deploy repo:
  - Locally create the bootstrap infra required to have an OIDC ready role for managing infra in your AWS account.
    ...
    > This should also create and configure your remote state.
...
- Using Terragrunt orchestrate and deploy your VPC and EKS module in the deploy repo
  - Write a workflow for the deploy repo that will execute your terragrunt plan and apply.
...
Wonderful! Now you have an EKS cluster stood up with IaC. Next, we are going to add ArgoCD to the cluster as our preferred method for deploying Kubernetes applications.
...
?> Something to consider: is deploying ArgoCD via terraform a good long term solution? Is it following proper GitOps principles?
  To fix this, once your platform is more developed consider trying to get ArgoCD to manage itself after it's initial install.
...
- Create a backstage template to onboard an 'app teams' application. Your template should do the following.
  ...
  - Create a PR to your deploy repo that adds `prod/applications/<app-name>/kustomization.yaml` and `prod/applications/<app-name>/application.yaml`
    > Your application.yaml should point to `prod/applications/<app-name>` at HEAD in your deploy repo. Your kustomization.yaml should point to the app teams repo at the version specified in the template.
    >
    > In order for this to work, your backstage deployment will need access to the credentials needed to make the PR. While creating Kubernetes secrets manually is an option, this is toilsome and gets even worse as platform scales and the number of secrets grows. Consider using an external secrets management solution such as AWS Secrets Manager or HashiCorp Vault.
    >
    > If you take this route you will need a way to get the secrets from the secrets manager to your cluster. Research how you can get Kubernetes to fetch secrets from a secrets manager.
    >
...
1) [Install Renovate](https://github.com/apps/renovate) into your org
...
In order to make the app healthy again you could manually create a Kubernetes secret. This is pretty common but the problem is this approach is not declarative. If you needed to rebuild the cluster there would be some manual intervention required. We can do better.

1) Recall how you configured your secrets for Exercise 4 and refer back to the hints if needed. If you haven't already, set up an external secrets manager and find a way to have your cluster ingest secrets from it.
2) Add the secrets needed to pull from the private container registry to your secrets manager.
3) Configure Kubernetes to get the ImagePullSecret.
