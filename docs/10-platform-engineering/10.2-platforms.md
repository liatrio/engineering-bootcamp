...
- Create a backstage template to onboard an 'app teams' application. Your template should do the following.
  ...
  - Create a PR to your deploy repo that adds `prod/applications/<app-name>/kustomization.yaml` and `prod/applications/<app-name>/application.yaml`
    ...
    > In order for this to work, your backstage deployment will need access to the credentials needed to make the PR. While creating kubernetes secrets manually is an option, this is toilsome and gets even worse as platform scales and the number of secrets grows. Consider using an external secrets managment solution such as AWS Secrets Manager or HashiCorp Vault.
...
3) Declaratively configure an ArgoCD Credential Template leveraging new secrets.
4) Add the secrets needed to pull from the private container registry to your secrets manager.
5) Configure Kubernetes to get the ImagePullSecret.
