...
- Create a backstage template to onboard an 'app teams' application. Your template should do the following.
  ...
  - Create a PR to your deploy repo that adds `prod/applications/<app-name>/kustomization.yaml` and `prod/applications/<app-name>/application.yaml`
    ...
    > If you take this route you will need a way to get the secrets from the secrets manager to your cluster. Research a solution for this.
...
1) If you haven't already, set up an external secrets manager and find a way to have your cluster ingest secrets from it.
